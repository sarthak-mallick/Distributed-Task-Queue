...
## Current Runbook (Completed Through Week 3 Day 15)
...
- `kubectl` installed (required for Week 3 Day 15 AKS/Jenkins validation checks)
...
- Week 3 Day 15 scaffold checks:
  - Jenkinsfile stage + shared image contract verification (`${ACR_LOGIN_SERVER}/dtq-<component>:${IMAGE_TAG}`)
  - AKS deploy helper verification (`infra/aks/scripts/deploy-release.sh`)
...
- skip Week 3 Day 15 scaffold checks: `bash scripts/run-current-e2e.sh --skip-week3-checks`
